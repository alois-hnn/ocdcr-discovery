...
from ocpa.util.dcr.converter import dcr_template
...
# Suppress warnings from pandas that result from using a pm4py version that is too old and that do not matter
...
def apply(log, findAdditionalConditions=True, parameters = None) -> DCRGraph:
    """
    ...
    """
    disc = Discover()
    graph, log = disc.mine(log, findAdditionalConditions, parameters = parameters)
    # TODO find out if nested works correct
    #graph = nested.apply(graph)
    return DCRGraph(graph)
...
class Discover:
    ...
    def mine(self, log: Union[EventLog, pd.DataFrame], findAdditionalConditions=True, parameters=None) :
        """
    ...
    def createLogAbstraction(self, log: [EventLog,pd.DataFrame], activity_key: str, case_key: str) -> int:
        ...
        #log = pm4py.project_on_event_attribute(log, attribute_key=case_key)
        ...
        df = format_dataframe(df,case_id = case_key,activity_key = activity_key,timestamp_key = xes_constants.DEFAULT_TIMESTAMP_KEY)

        traces = pm4py.project_on_event_attribute(df, attribute_key = activity_key)

        # flatten the event log, all traces are equally significant
        traces = set(tuple(i) for i in traces)
